...
class DeviceAutomation {
  ...
  async triggerLockdown(alarmName = "Base Alarm", entityId = null, source = "alarm") {
    // If this alarm is a monument RF alarm (Oil Rig / Cargo), do not trigger base lockdown!
    if (source !== "manual" && this.rustClient?.monumentTracker?.isMonumentAlarm(alarmName)) {
      console.log(`[DeviceAutomation] Ignoring compound lockdown for monument alarm: "${alarmName}"`);
      return { success: false, reason: "monument_alarm", formattedText: `Monument Alarm "${alarmName}" routed to Monument Tracker.` };
    }
