...
class FCMService extends EventEmitter {
  ...
  async registerFCM() {
    ...
    const projectId = process.env.FCM_PROJECT_ID || config.fcm?.projectId || "rust-companion-app";
    const gcmSenderId = process.env.FCM_GCM_SENDER_ID || config.fcm?.gcmSenderId || "REMOVED_GCM_SENDER_ID";
    const gmsAppId = process.env.FCM_GMS_APP_ID || config.fcm?.gmsAppId || "REMOVED_GMS_APP_ID";
    const androidPackageName = process.env.FCM_PACKAGE_NAME || config.fcm?.androidPackageName || "com.facepunch.rust.companion";
    const androidPackageCert = process.env.FCM_PACKAGE_CERT || config.fcm?.androidPackageCert || "REMOVED_PACKAGE_CERT";
    const expoProjectId = process.env.EXPO_PROJECT_ID || config.fcm?.expoProjectId || "REMOVED_EXPO_PROJECT_ID";

    if (!apiKey) {
      const errMsg = "FCM registration failed: FCM_API_KEY is not defined in environment variables or .env file.";
