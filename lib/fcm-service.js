...
class FCMService extends EventEmitter {
  ...
  async registerFCM() {
    ...
    const apiKey = "REMOVED_FCM_API_KEY";
    const projectId = "rust-companion-app";
    const gcmSenderId = "REMOVED_GCM_SENDER_ID";
    const gmsAppId = "REMOVED_GMS_APP_ID";
    const androidPackageName = "com.facepunch.rust.companion";
    const androidPackageCert = "REMOVED_PACKAGE_CERT";
    ...
    const expoResponse = await axios.post("https://exp.host/--/api/v2/push/getExpoPushToken", {
      ...
      projectId: "REMOVED_EXPO_PROJECT_ID"
    });
  ...
  handleIncomingNotification(data) {
    try {
      ...
      const isEntityPairing = notifType === "entity" || 
                              Boolean(payload.entityId || payload.body?.entityId || appMap.entityId || data.entityId) ||
                              Boolean(payload.entityType || payload.body?.entityType || appMap.entityType || data.entityType);

      if (isEntityPairing) {
        const entData = payload.body || payload;
        const entityId = Number(entData.entityId || payload.entityId || appMap.entityId || data.entityId);
        const rawType = Number(entData.entityType || payload.entityType || appMap.entityType || data.entityType || 1); // 1 = Switch, 2 = Alarm, 3 = Storage
