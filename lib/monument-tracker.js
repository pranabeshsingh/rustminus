...
class MonumentTracker {
  ...
  /**
   * Identifies whether a given alarm name or string relates to Large Oil, Small Oil, or Cargo.
   */
  ...
  /**
   * Determine monument key from text identifiers.
   */
  classifyMonument(text) {
    const s = String(text || "").toLowerCase();
    if (s.includes("large") || s.includes("loil") || s.includes("4765") || s.includes("l-oil") || s.includes("l_oil") || s.includes("l rig")) {
      return "large_oil";
    }
    if (s.includes("small") || s.includes("smoil") || s.includes("4768") || s.includes("s-oil") || s.includes("s_oil") || s.includes("s rig")) {
      return "small_oil";
    }
    if (s.includes("cargo")) {
      return "cargo";
    }
    // Generic "oil rig" or "oil" defaults to large_oil if no specific prefix
  ...
  formatChatSummary() {
    ...
    return `[Monument Timers] ${loil} | ${smoil} | ${cargo}`;
  }
}
