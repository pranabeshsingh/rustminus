...
class CommandProcessor {
  constructor(rustClient, matrixClient, options = {}) {
    ...
    this.monumentTracker = options.monumentTracker || rustClient?.monumentTracker || null;
  ...
  async execute(cmd, args, source, sender) {
    switch (cmd) {
      ...
      case "players":
      case "player": {
        if (!this.rustClient.client?.isConnected()) return "Not connected to an active Rust server.";
        // Always fetch fresh data so the count is never stale
        try { await this.rustClient.fetchServerInfo(); } catch (_) {}
        const info = this.rustClient.serverInfo;
        if (!info) return "Server population unavailable.";
        const current = info.players || 0;
        const base = `[Pop] ${current}/${info.maxPlayers || 0} Online | Queue: ${info.queuedPlayers || 0} | ${info.name || "Server"}`;
        // Compute delta vs ~1 hour ago (nearest entry within 45–75 min window)
        const history = this.rustClient.popHistory || [];
        const now = Date.now();
        const oneHourAgo = now - 60 * 60 * 1000;
        const candidates = history.filter(e => {
          const age = now - e.timestamp;
          return age >= 45 * 60 * 1000 && age <= 75 * 60 * 1000;
        });
        let deltaStr = "";
        if (candidates.length > 0) {
          // Pick the entry closest to exactly 1h ago
          const nearest = candidates.reduce((a, b) =>
            Math.abs(a.timestamp - oneHourAgo) <= Math.abs(b.timestamp - oneHourAgo) ? a : b
          );
          const delta = current - nearest.players;
          if (delta > 0) deltaStr = ` | +${delta} from last hour`;
          else if (delta < 0) deltaStr = ` | ${delta} from last hour`;
          else deltaStr = " | same as last hour";
        }
      ...
      case "paircodes":
      case "lastpair": {
        if (!args || args.trim() === "code" || args.trim() === "codes" || args.trim() === "last") {
          const recents = this.rustClient.recentPairCodes || [];
          if (recents.length > 0) {
            const latest = recents[0];
            const agoSec = Math.max(0, Math.round((Date.now() - latest.timestamp) / 1000));
            const timeAgo = agoSec < 60 ? `${agoSec}s ago` : `${Math.round(agoSec / 60)}m ago`;
            const list = recents.slice(0, 3).map(r => `Code: ${r.entityId} ("${r.entityName}" - ${r.type})`).join(" | ");
            return `[Pair Code] Latest: ${latest.entityId} ("${latest.entityName}" - ${latest.type}, ${timeAgo}). Recent: ${list}. To register: !pair <id> <name> [switch|alarm|storage]`;
          }
          return "No recent pairing codes detected. In Rust, look at a Smart Switch/Alarm/TC and push 'Pair Device', or pair manually: !pair <id> <name> [switch|alarm|storage]";
        }

        const parts = args.trim().split(/\s+/);
        const entityId = Number(parts[0]);
        if (isNaN(entityId) || entityId <= 0) {
          return "Usage: !pair <entityId> <name> [switch|alarm|storage]  (e.g. !pair 12345678 Base Alarm alarm)";
        }
        ...
        if (parts.length > 2) {
          ...
          if (["alarm", "switch", "storage", "tc", "box"].includes(lastWord)) {
            type = lastWord;
            name = parts.slice(1, -1).join(" ");
          }
        }
        if (!name) name = `Device ${entityId}`;

        try {
          const res = this.rustClient.pairEntity(entityId, name, type);
          return `[Pair] Successfully paired "${res.name}" (ID: ${res.entityId}) as ${res.type}! Auto-subscribed.`;
        } catch (e) {
      ...
      case "paired":
      case "devices": {
        if (!this.rustClient.activeServer) return "Not connected to an active Rust server.";
        const s = this.rustClient.activeServer;
        const sws = s.switches || [];
        const als = s.alarms || [];
        const stos = s.storageMonitors || [];
        if (sws.length === 0 && als.length === 0 && stos.length === 0) {
          return "No devices paired to this server. Use !pair <id> <name> [type] or push Pair in-game.";
        }
        const sections = [];
        if (sws.length > 0) sections.push(`Switches (${sws.length}): ${sws.map(x => `${x.name} [${x.id}]`).join(", ")}`);
        if (als.length > 0) sections.push(`Alarms (${als.length}): ${als.map(x => `${x.name} [${x.id}]`).join(", ")}`);
        if (stos.length > 0) sections.push(`Storage (${stos.length}): ${stos.map(x => `${x.name} [${x.id}]`).join(", ")}`);
        return `[Paired Devices] ${sections.join(" | ")}`;
      }

      case "unpair": {
        if (!args) return "Usage: !unpair <entityId or device name>";
        try {
          const removed = this.rustClient.unpairEntity(args);
          return `[Unpair] Successfully removed "${removed.name}" (ID: ${removed.id}) [${removed.deviceType}] from paired devices.`;
        } catch (e) {
          return `[Unpair Error] ${e.message}`;
        }
      }

      case "rename":
      case "setname": {
        if (!args) return "Usage: !rename <entityId or current name> <new name>";
        const parts = args.trim().split(/\s+/);
        if (parts.length < 2) return "Usage: !rename <entityId or current name> <new name>";
        const targetId = parts[0];
        const newName = parts.slice(1).join(" ");
        try {
          const res = this.rustClient.renameEntity(targetId, newName);
          return `[Rename Device] Successfully renamed ID ${res.entityId} to "${res.name}" [${res.deviceType}].`;
        } catch (e) {
          return `[Rename Error] ${e.message}`;
        }
      }

      // =========================================================================
      // 3. SMART SWITCHES & AUTOMATIONS
      // =========================================================================
      case "on": {
      ...
      case "lockdown":
      case "lock": {
        if (!this.deviceAutomation) return "⚠️ Device automation unavailable.";
        const sub = (args || "on").toLowerCase().trim();
        if (sub === "off" || sub === "cancel" || sub === "standdown") {
          return await this.deviceAutomation.cancelLockdown();
        }
        if (sub === "status") {
          const st = this.deviceAutomation.getLockdownStatus();
          return st.active ? `🚨 [Lockdown Active] Triggered ${st.timeAgo} due to ${st.reason}.` : "🟢 Compound defense status: Normal (Lockdown inactive).";
        }
        return (await this.deviceAutomation.triggerLockdown(`Manual Squad Alert by ${sender}`, null, "manual")).formattedText;
      }
      ...
      case "squads": {
        if (!this.teamTracker) return "⚠️ Team tracker unavailable.";
        return this.teamTracker.getSquadSummary(args);
      }
      ...
      case "assignsquad": {
        if (!this.teamTracker) return "⚠️ Team tracker unavailable.";
        const parts = args.split(" ");
        if (parts.length < 2) return "Usage: !setsquad <player name or steamId> <squad name>";
        const res = this.teamTracker.assignSquad(parts[0], parts.slice(1).join(" "));
        return res.success ? `🏰 ${res.message}` : `⚠️ ${res.error}`;
      }
      ...
      case "onlinealert": {
        if (!this.teamTracker) return "⚠️ Team tracker unavailable.";
        const cfg = this.teamTracker.getAlertConfig();
        const cleanArg = (args || "").trim().toLowerCase();

        if (!cleanArg) {
          const status = cfg.enabled ? "🟢 ENABLED" : "🔴 DISABLED";
          const thresh = cfg.minOfflineHours !== undefined ? cfg.minOfflineHours : 1.0;
          return `👥 [Team Reconnect Alert] Status: ${status} | Min Offline: ${thresh}h | In-Game: ${cfg.inGameTeamChat ? "ON" : "OFF"} | Clan: ${cfg.clanChat ? "ON" : "OFF"} | Matrix: ${cfg.matrixAlerts ? "ON" : "OFF"}`;
        }

        if (cleanArg === "on" || cleanArg === "enable") {
          this.teamTracker.setAlertConfig({ enabled: true });
          return `👥 [Team Reconnect Alert] Notifications ENABLED (Threshold: ${cfg.minOfflineHours || 1.0}h).`;
        }

        if (cleanArg === "off" || cleanArg === "disable") {
          this.teamTracker.setAlertConfig({ enabled: false });
          return `👥 [Team Reconnect Alert] Notifications DISABLED.`;
        }

        if (cleanArg === "test") {
          const caller = this.rustClient.teamInfo?.members?.find(m => m.name === sender) || { steamId: "76561198000000000", name: sender };
          this.teamTracker.triggerReconnectAlert(caller.name || sender, caller.steamId, 4.5, Date.now() - (4.5 * 3600 * 1000));
          return `👥 Dispatched test reconnect notification for ${sender} (Simulated 4.5 hours offline).`;
        }

        const num = parseFloat(cleanArg);
        if (!isNaN(num) && num >= 0) {
          const updated = this.teamTracker.setAlertConfig({ enabled: true, minOfflineHours: Math.round(num * 10) / 10 });
          return `👥 [Team Reconnect Alert] Min offline threshold set to ${updated.minOfflineHours} hour(s). Notifications are ENABLED.`;
        }

        return `Usage: !teamalert [on|off|<hours>|test] (e.g. !teamalert 2 or !teamalert test)`;
      }
      ...
      case "canweraid":
      case "raid": {
        if (!args) return "Usage: !raidcost <target> (e.g. !raidcost 3 garage doors, 1 sheet metal door)";
        const armoryTotals = this.storageTracker ? this.storageTracker.getClanArmoryTotals() : null;
        const res = GameDatabase.calculateRaidCost(args, armoryTotals);
        return res.formattedText || res.error || "⚠️ Raid cost calculation failed.";
      }
      ...
      // =========================================================================
      // OIL RIG & CARGO TIMERS (SMART ALARM + RF RECEIVER TRACKING)
      // =========================================================================
      case "oil":
      case "rig":
      case "rigs":
      case "timers":
      case "monuments": {
        if (!this.monumentTracker) return "Monument timer tracking is unavailable.";
        const sub = (args || "").toLowerCase().trim();

        if (sub === "rf" || sub === "freq" || sub === "freqs") {
          return "[Monument RF Frequencies] Large Oil Rig: 4765 | Small Oil Rig: 4768 (Wire RF Receiver to a Smart Alarm in base to auto-track!)";
        }

        if (sub === "large" || sub === "loil" || sub === "largeoil") {
          const s = this.monumentTracker.getStatus().large_oil;
          return `[Large Oil Rig] Status: ${s.formatted} (RF Frequency: ${s.rfFrequency}). Use '!oil large start' to manually trigger.`;
        }

        if (sub === "small" || sub === "smoil" || sub === "smalloil") {
          const s = this.monumentTracker.getStatus().small_oil;
          return `[Small Oil Rig] Status: ${s.formatted} (RF Frequency: ${s.rfFrequency}). Use '!oil small start' to manually trigger.`;
        }

        if (sub.startsWith("large start") || sub.startsWith("loil start") || sub.startsWith("start large")) {
          this.monumentTracker.startTimer("large_oil", `Manual by ${sender || "Player"}`);
          return "[Oil Rig Tracker] Started 15-minute countdown for Large Oil Rig crate hack!";
        }

        if (sub.startsWith("small start") || sub.startsWith("smoil start") || sub.startsWith("start small")) {
          this.monumentTracker.startTimer("small_oil", `Manual by ${sender || "Player"}`);
          return "[Oil Rig Tracker] Started 15-minute countdown for Small Oil Rig crate hack!";
        }

        if (sub.startsWith("cargo start") || sub.startsWith("start cargo")) {
          this.monumentTracker.startTimer("cargo", `Manual by ${sender || "Player"}`);
          return "[Cargo Ship Tracker] Started 15-minute countdown for Cargo Ship crate hack!";
        }

        if (sub.startsWith("cancel") || sub.startsWith("reset")) {
          const target = sub.split(/\s+/)[1] || "all";
          if (target === "large" || target === "loil") {
            this.monumentTracker.cancelTimer("large_oil");
            return "[Oil Rig Tracker] Large Oil Rig timer reset.";
          } else if (target === "small" || target === "smoil") {
            this.monumentTracker.cancelTimer("small_oil");
            return "[Oil Rig Tracker] Small Oil Rig timer reset.";
          } else if (target === "cargo") {
            this.monumentTracker.cancelTimer("cargo");
            return "[Cargo Ship Tracker] Cargo Ship timer reset.";
          } else {
            this.monumentTracker.cancelTimer("large_oil");
            this.monumentTracker.cancelTimer("small_oil");
            this.monumentTracker.cancelTimer("cargo");
            return "[Monument Tracker] All monument timers reset to idle.";
          }
        }

        return this.monumentTracker.formatChatSummary();
      }

      case "loil":
      case "largeoil": {
        if (!this.monumentTracker) return "Monument tracker unavailable.";
        const sub = (args || "").toLowerCase().trim();
        if (sub === "start" || sub === "hack") {
          this.monumentTracker.startTimer("large_oil", `Manual by ${sender || "Player"}`);
          return "[Oil Rig Tracker] Started 15-minute countdown for Large Oil Rig crate hack!";
        }
        const s = this.monumentTracker.getStatus().large_oil;
        return `[Large Oil Rig] ${s.formatted} (RF Frequency: ${s.rfFrequency}). Use '!loil start' to trigger manually.`;
      }
      ...
      case "smalloil": {
        ...
        const s = this.monumentTracker.getStatus().small_oil;
        return `[Small Oil Rig] ${s.formatted} (RF Frequency: ${s.rfFrequency}). Use '!smoil start' to trigger manually.`;
      }
      ...
      case "cargoship": {
        ...
        if (sub === "start" || sub === "hack") {
          this.monumentTracker.startTimer("cargo", `Manual by ${sender || "Player"}`);
          return "[Cargo Ship Tracker] Started 15-minute countdown for Cargo Ship locked crate!";
        }
        const s = this.monumentTracker.getStatus().cargo;
        const mapLoc = s.onMap ? `Active @ [${s.grid || "Water"}]` : "Not on map";
        return `[Cargo Ship] ${mapLoc} | ${s.formatted}. Use '!cargo start' to trigger crate timer.`;
      }
      ...
      case "excavator": {
        ...
        if (sub === "start" || sub === "run") {
          this.monumentTracker.startTimer("excavator", `Manual by ${sender || "Player"}`);
          return "[Giant Excavator] Started Excavator run timer!";
        }
        const s = this.monumentTracker.getStatus().excavator;
        return `[Giant Excavator] Status: ${s ? s.formatted : "Idle"} (RF Frequency: 4777). Use '!exac start' to trigger timer.`;
      }
      ...
      case "camera":
      case "cameras": {
        return GameDatabase.getMonumentCameras(args);
      }
      ...
      // =========================================================================
      // 8b. BASE CODES & SECURITY (!codes, !code)
      // =========================================================================
      case "codes":
      case "code":
      case "basecode":
      case "basecodes": {
        ...
        // Base codes must NEVER be output to clan chat under any circumstance!
        if (source === "clan") {
          return "[Security] Base codes and secrets can ONLY be requested in Team Chat, never in Clan Chat.";
        }

        const codes = (this.rustClient && typeof this.rustClient.getBaseCodes === "function")
          ? this.rustClient.getBaseCodes()
          : [];

        if (!codes || codes.length === 0) {
          return "ℹ️ No base codes stored. Add your base codes and info in the WebUI Base Codes tab.";
        }

        // If a specific base was asked for (e.g. "!codes main" or "!codes farm")
        if (args) {
          const query = args.toLowerCase().trim();
          const match = codes.find(c => 
            (c.name && c.name.toLowerCase() === query) ||
            (c.name && c.name.toLowerCase().includes(query)) ||
            (c.grid && c.grid.toLowerCase() === query)
          );

          if (!match) {
            const names = codes.map(c => c.name || "Unnamed").join(", ");
            return `⚠️ Base "${args}" not found. Stored bases: ${names}. Use !codes to list all.`;
          }

          const parts = [];
          if (match.doorCode) parts.push(`Door: ${match.doorCode}`);
          if (match.turretCode) parts.push(`Turret: ${match.turretCode}`);
          if (match.guestCode) parts.push(`Guest: ${match.guestCode}`);
          if (match.tcCode) parts.push(`TC: ${match.tcCode}`);
          if (match.notes) parts.push(`Notes: ${match.notes}`);

          const gridStr = match.grid ? ` @ [${match.grid}]` : "";
          const details = parts.length > 0 ? parts.join(" | ") : "No codes saved";
          return `🔐 [Code: ${match.name}${gridStr}] ${details}`;
        }

        // If only 1 base exists, print all details directly
        if (codes.length === 1) {
          const b = codes[0];
          const parts = [];
          if (b.doorCode) parts.push(`Door: ${b.doorCode}`);
          if (b.turretCode) parts.push(`Turr: ${b.turretCode}`);
          if (b.guestCode) parts.push(`Guest: ${b.guestCode}`);
          if (b.tcCode) parts.push(`TC: ${b.tcCode}`);
          if (b.notes) parts.push(`Notes: ${b.notes}`);
          const gridStr = b.grid ? ` @ [${b.grid}]` : "";
          return `🔐 [Base Code: ${b.name}${gridStr}] ${parts.join(" | ")}`;
        }

        // Multiple bases: provide compact list fitting in Rust in-game chat
        const summaries = codes.map(b => {
          const items = [];
          if (b.doorCode) items.push(`Door ${b.doorCode}`);
          if (b.turretCode) items.push(`Turr ${b.turretCode}`);
          const gridStr = b.grid ? ` [${b.grid}]` : "";
          return `${b.name}${gridStr}: ${items.join(", ") || "Configured"}`;
        });

        return `🔐 [Base Codes] ${summaries.join(" | ")} (!codes <name> for details)`;
      }
      ...
      case "help":
      case "commands": {
        return "📖 Commands: !pop, !time, !daynight, !oil [large|small|cargo|rf], !cargo, !clan, !pair [id name], !paired, !unpair <id>, !codes [name], !squad <name>, !setsquad <p> <s>, !team, !teamalert [h], !locate <name>, !lockdown [on/off], !raidcost <targets>, !boom, !sulfur, !armory, !multitc, !cams [monument], !motd, !turrets on/off, !sams on/off, !doors on/off, !craft <qty> <item>, !recycle <item>, !ai <msg>, !status";
      }
