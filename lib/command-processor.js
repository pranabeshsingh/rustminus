  ...
  async execute(cmd, args, source, sender) {
    switch (cmd) {
      ...
      case "time": {
        ...
        return `${isDay ? "☀️" : "🌙"} [Time] ${this.formatTime(t.time)} | Sunrise: ${this.formatTime(t.sunrise)} | Sunset: ${this.formatTime(t.sunset)} (${isDay ? "Day" : "Night"})`;
      }
      ...
      case "day": {
        ...
        if (t.time >= t.sunrise && t.time < t.sunset) {
          const hoursLeft = t.sunset - t.time;
          const mins = Math.round(hoursLeft * 60 / (t.timeScale || 1));
          return `☀️ [Day Time] Day is active (${this.formatTime(t.time)}). Night begins in ~${mins}m at ${this.formatTime(t.sunset)}.`;
        } else {
          const hoursUntilDay = (t.time < t.sunrise) ? (t.sunrise - t.time) : (24 - t.time + t.sunrise);
          const mins = Math.round(hoursUntilDay * 60 / (t.timeScale || 1));
      ...
      case "night": {
        ...
        if (!t) return "⚠️ Time data unavailable.";
        if (t.time < t.sunrise || t.time >= t.sunset) {
          return `🌙 [Night Active] Current clock: ${this.formatTime(t.time)}. Sunrise at ${this.formatTime(t.sunrise)}.`;
        } else {
          const hoursUntil = t.sunset - t.time;
          const mins = Math.round(hoursUntil * 60 / (t.timeScale || 1));
      ...
      case "help":
      case "commands": {
        return "📖 Commands: !pop, !time, !day, !night, !now, !team, !locate <name>, !nearby, !proximity, !afk, !death, !upkeep, !contents <box>, !contains <item>, !turrets on/off, !sams on/off, !on/off <name>, !ttoggle <time> <name>, !auto-on/off, !durability <item>, !craft <qty> <item>, !recycle <item>, !monument <name>, !timer <time> <msg>, !note <name> <msg>, !steamid <name>, !ai <msg>, !status";
      }
