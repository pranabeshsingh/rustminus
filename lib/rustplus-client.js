...
const TimeNotifier = require("./time-notifier");
const MonumentTracker = require("./monument-tracker");
...
class RustPlusManager extends EventEmitter {
  ...
  constructor(matrixClient, getServersCallback, saveServersCallback, configManager = null) {
    ...
    this.teamInfo = null;
    this.timeInfo = null;
    this.popHistory = []; // [{timestamp: ms, players: n}] rolling 2-hour window
    this.cachedMap = null;
    ...
    this.itemsMap = null;
    this.clanInfo = null;
    this.clanChatHistory = [];
    this.recentPairCodes = []; // [{ entityId, entityName, type, serverName, timestamp }]
    ...
    this.timeNotifier = new TimeNotifier(this, matrixClient, configManager);
    this.monumentTracker = new MonumentTracker(this, matrixClient);
    ...
    this.commandProcessor = new CommandProcessor(this, matrixClient, {
      ...
      timeNotifier: this.timeNotifier,
      monumentTracker: this.monumentTracker
    });
    this.turretTracker = this.commandProcessor.turretTracker;
  }
  ...
  async connect() {
    ...
    try {
      ...
      this.client.on("connected", async () => {
        ...
        // Broadcast welcome message in team chat so team members know bot is online
        try {
          await this.sendTeamChat("Welcome to Rust");
          console.log("[RustPlus] Sent connection announcement: 'Welcome to Rust'");
        } catch (err) {
          console.warn("[RustPlus] Could not send welcome greeting to team chat:", err.message);
        }

        // Subscribe to all configured smart switches / alarms
        this.subscribeConfiguredEntities();

        // Start map marker, team and time polling loop (every 15s)
        // Server info (population) is refreshed every 60s (every 4th tick)
        if (this.pollInterval) clearInterval(this.pollInterval);
        let pollTick = 0;
        this.pollInterval = setInterval(async () => {
          await this.pollMapMarkers();
          await this.pollTeamInfo();
          await this.pollTimeInfo();
          pollTick++;
          if (pollTick % 4 === 0) {
  ...
  async refreshData() {
    ...
    try {
      ...
      // Clan info may not be supported on this server
    }
  }
  ...
  async fetchClanInfo() {
    ...
    return new Promise((resolve) => {
      try {
        this.client.getClanInfo((res) => {
          const info = res?.clanInfo?.clanInfo || res?.response?.clanInfo?.clanInfo;
          if (info) {
            this.clanInfo = info;
            this.emit("clanInfo", this.clanInfo);
            resolve(this.clanInfo);
          } else {
            resolve(this.clanInfo);
          }
        });
      } catch (e) {
        console.warn("[RustPlus] fetchClanInfo error:", e.message);
        resolve(null);
      }
    });
  }
  ...
  async fetchClanChat() {
    if (!this.client || !this.client.isConnected()) return this.clanChatHistory || [];
    return new Promise((resolve) => {
      try {
        this.client.getClanChat((res) => {
          const raw = res?.clanChat?.messages || res?.response?.clanChat?.messages || [];
          this.clanChatHistory = raw.map(m => ({
            sender: m.name || (m.steamId ? String(m.steamId) : "Clanmate"),
            message: m.message,
            steamId: m.steamId ? String(m.steamId) : null,
            time: m.time ? Number(m.time) * 1000 : Date.now()
          }));
          this.emit("clanChat", this.clanChatHistory);
          resolve(this.clanChatHistory);
        });
      } catch (e) {
        console.warn("[RustPlus] fetchClanChat error:", e.message);
        resolve(this.clanChatHistory || []);
      }
    });
  }
  ...
  isClanChatEnabled() {
    try {
      ...
      if (!cfg) return false;
      return !!(cfg.clanChat && cfg.clanChat.enabled);
    } catch (e) {
      return false;
    }
  }
  ...
  getBaseCodes() {
    try {
      const p = path.join(__dirname, "..", "data", "base_codes.json");
      if (fs.existsSync(p)) {
        return JSON.parse(fs.readFileSync(p, "utf8"));
      }
    } catch (e) {
      console.error("[BaseCodes] Error reading base codes:", e.message);
    }
    return [];
  }
  ...
  saveBaseCodes(codes) {
    try {
      ...
      console.error("[BaseCodes] Error saving base codes:", e.message);
      return false;
    }
  }

  async sendClanMessage(message, force = false) {
    if (!force && !this.isClanChatEnabled()) {
      console.log(`[ClanChat] Suppressed clan chat message (clanChat responses/messages disabled in settings): "${this.stripEmoji(message)}"`);
      return { success: false, suppressed: true };
    }
    if (!this.client || !this.client.isConnected()) {
      throw new Error("Cannot send clan chat: Not connected to Rust+ server");
    }
    const safeMsg = this.stripEmoji(message);
    return new Promise((resolve, reject) => {
      try {
        this.client.sendClanMessage(safeMsg, (res) => {
          if (res?.response?.error) {
            return reject(new Error(res.response.error.error || "Failed to send clan message"));
          }
  ...
  async setClanMotd(motd) {
    if (!this.client || !this.client.isConnected()) {
      throw new Error("Cannot set clan MOTD: Not connected to Rust+ server");
    }
    return new Promise((resolve, reject) => {
      try {
        this.client.setClanMotd(motd, (res) => {
          if (res?.response?.error) {
            return reject(new Error(res.response.error.error || "Failed to set clan MOTD"));
          }
          if (this.clanInfo) {
            this.clanInfo.motd = motd;
            this.clanInfo.motdTimestamp = Math.floor(Date.now() / 1000);
          }
          this.emit("clanInfo", this.clanInfo);
          this.logEvent("clan", "Clan MOTD Updated", motd);
          resolve({ success: true, motd });
        });
      } catch (e) {
        reject(e);
      }
  ...
  async fetchServerInfo() {
    ...
    if (info) {
      ...
      // Record population snapshot for hourly-delta tracking
      const now = Date.now();
      this.popHistory.push({ timestamp: now, players: info.players || 0 });
      // Keep only the last 2 hours of snapshots
      const cutoff = now - 2 * 60 * 60 * 1000;
      this.popHistory = this.popHistory.filter(e => e.timestamp >= cutoff);
  ...
  recordPairCode(entityId, entityName, type, serverName) {
    const entry = {
      entityId: Number(entityId),
      entityName: entityName || `Smart Device ${entityId}`,
      type: type || "Smart Device",
      serverName: serverName || this.activeServer?.name || "Server",
      timestamp: Date.now()
    };
    // Keep newest 10 codes, deduplicate by entityId
    this.recentPairCodes = [entry, ...this.recentPairCodes.filter(e => e.entityId !== entry.entityId)].slice(0, 10);
    return entry;
  }

  pairEntity(entityId, name, type = "switch", category = null) {
    if (!this.activeServer) {
      throw new Error("No active server connected to pair device with.");
    }
    const eId = Number(entityId);
    if (!eId || isNaN(eId) || eId <= 0) {
      throw new Error("Invalid entity ID. Must be a positive numeric ID.");
    }

    const lowerName = (name || "").toLowerCase();
    const lowerType = (type || "").toLowerCase();

    // Auto-detect or parse final type
    let finalType = "switch";
    if (lowerType.includes("alarm") || lowerName.includes("alarm")) {
      finalType = "alarm";
    } else if (
    ...
    if (finalType === "alarm") {
      ...
      if (lowerName.includes("turret")) cat = "Turrets";
      else if (lowerName.includes("sam")) cat = "SAMs";
      else if (lowerName.includes("light")) cat = "Lights";
      else if (lowerName.includes("door")) cat = "Doors";
    ...
    const allServers = this.getServers ? this.getServers() : [];
    const idx = allServers.findIndex(s => s.id === server.id);
    if (idx !== -1) {
      allServers[idx] = server;
      if (this.saveServers) this.saveServers(allServers);
    }

    // Subscribe immediately and update recent codes
    this.subscribeEntity(eId);
    this.recordPairCode(eId, name, label, server.name);

    this.logEvent("entity", `Paired ${label}: "${name}" (ID: ${eId})`, { id: eId, type: label });
    return { success: true, entityId: eId, name, type: label };
  }

  unpairEntity(identifier) {
    if (!this.activeServer) {
      throw new Error("No active server connected.");
    }
    const server = this.activeServer;
    ...
    let removed = null;
    if (server.switches) {
      const idx = server.switches.findIndex(s => Number(s.id) === numId || s.name.toLowerCase() === lower || s.name.toLowerCase().includes(lower));
      if (idx !== -1) {
        removed = server.switches.splice(idx, 1)[0];
        removed.deviceType = "Smart Switch";
      }
    }
    if (!removed && server.alarms) {
      const idx = server.alarms.findIndex(a => Number(a.id) === numId || a.name.toLowerCase() === lower || a.name.toLowerCase().includes(lower));
      if (idx !== -1) {
        removed = server.alarms.splice(idx, 1)[0];
        removed.deviceType = "Smart Alarm";
      }
    }
    if (!removed && server.storageMonitors) {
      const idx = server.storageMonitors.findIndex(s => Number(s.id) === numId || s.name.toLowerCase() === lower || s.name.toLowerCase().includes(lower));
      if (idx !== -1) {
        removed = server.storageMonitors.splice(idx, 1)[0];
        removed.deviceType = "Storage Monitor";
      }
    }

    if (!removed) {
      throw new Error(`Device "${identifier}" not found in paired devices.`);
    }
    ...
    this.logEvent("entity", `Unpaired Device: "${removed.name}" (ID: ${removed.id})`, removed);
    return removed;
  }

  renameEntity(identifier, newName, type = null, category = null) {
    if (!this.activeServer) {
      throw new Error("No active server connected.");
    }
    const server = this.activeServer;
    ...
    if (!cleanName) throw new Error("New device name cannot be empty.");
    ...
    if (server.switches) {
      target = server.switches.find(s => Number(s.id) === numId || s.name.toLowerCase() === lower);
      if (target) {
    ...
    if (!target && server.alarms) {
      target = server.alarms.find(a => Number(a.id) === numId || a.name.toLowerCase() === lower);
      if (target) {
        target.name = cleanName;
        oldType = "alarm";
    ...
    if (!target && server.storageMonitors) {
      target = server.storageMonitors.find(s => Number(s.id) === numId || s.name.toLowerCase() === lower);
      if (target) {
        target.name = cleanName;
        oldType = "storage";
    ...
    // If type was changed explicitly, migrate entity across arrays
    if (type && type !== oldType) {
      const eId = Number(target.id);
    ...
    // Update storageTracker container name cache if exists
    if (this.storageTracker?.containers.has(Number(target.id))) {
      const c = this.storageTracker.containers.get(Number(target.id));
      c.name = cleanName;
    }
    ...
    this.logEvent("entity", `Renamed Device: "${cleanName}" (ID: ${target.id}, Type: ${deviceType})`, { id: target.id, name: cleanName, type: deviceType });
    return { success: true, entityId: target.id, name: cleanName, deviceType };
  }
  ...
  subscribeConfiguredEntities() {
    ...
    const allEntities = [
      ...
      ...(this.activeServer.storageMonitors || [])
    ];

    for (const entity of allEntities) {
      if (entity.id) {
        this.client.getEntityInfo(Number(entity.id), (res) => {
          const payload = res?.response?.entityInfo?.payload || res?.entityInfo?.payload;
          if (payload) {
            const val = !!payload.value;
            entity.state = val;
            this.emit("entityState", { serverId: this.activeServer.id, entityId: entity.id, state: val });
            if (Array.isArray(payload.items)) {
              this.emit("entityChanged", { entityId: entity.id, payload });
            }
          }
        });
      }
  ...
  async handleIncomingMessage(message) {
    ...
    // 2. In-game Clan Chat message broadcast
    let clanMsg = null;
    if (message.broadcast?.clanMessage?.message) {
      clanMsg = message.broadcast.clanMessage.message;
    } else if (message.broadcast?.clanMessage) {
      clanMsg = message.broadcast.clanMessage;
    }

    if (clanMsg) {
      const senderName = clanMsg.name || (clanMsg.steamId ? String(clanMsg.steamId) : "Clanmate");
      const content = String(clanMsg.message || (typeof clanMsg === "string" ? clanMsg : "")).trim();
      const time = clanMsg.time ? Number(clanMsg.time) * 1000 : Date.now();

      console.log(`[RustPlus ClanChat] ${senderName}: "${content}"`);
      this.logEvent("clanChat", `Clan Chat: ${senderName}`, content, { sender: senderName });

      const chatItem = {
        sender: senderName,
        message: content,
        time,
        steamId: clanMsg.steamId ? String(clanMsg.steamId) : null
      };
      if (!this.clanChatHistory) this.clanChatHistory = [];
      this.clanChatHistory.push(chatItem);
      if (this.clanChatHistory.length > 100) this.clanChatHistory.shift();

      this.emit("clanMessage", chatItem);

      // Forward to Matrix Team Chat room with Clan tag
      if (this.matrixClient) {
        try {
          await this.matrixClient.sendTeamChat(`🏰 [Clan] ${senderName}`, content, "#ffaa00");
        } catch (e) {
          console.error("[Matrix Relay] Failed to forward clan chat:", e.message);
        }
      }

      // Check and handle in-game clan commands
      if ((content.startsWith("!") || content.startsWith(".")) && this.commandProcessor) {
        if (!this.isClanChatEnabled()) {
          console.log(`[ClanChat] Ignoring clan command "${content}" from ${senderName} because clan chat bot responses are disabled in settings.`);
          return;
        }
        try {
          console.log(`[Command Processor] Executing clan command: "${content}" from ${senderName}`);
          const reply = await this.commandProcessor.handleCommand(content, "clan", senderName);
          if (reply) {
            console.log(`[Command Reply] Dispatching clan chat response: "${reply}"`);
            await this.sendClanMessage(reply);
            if (this.matrixClient) {
              await this.matrixClient.sendTeamChat("Rust+ Sentinel", `🏰 ${reply}`, "#00ffff");
            }
          }
        } catch (err) {
          console.error("[Command Processor] Clan execution error:", err.message);
        }
      }
    }

    // Clan Changed Broadcast
    if (message.broadcast?.clanChanged?.clanInfo) {
      this.clanInfo = message.broadcast.clanChanged.clanInfo;
      console.log(`[RustPlus ClanChanged] Clan updated: ${this.clanInfo.name || "Clan"}`);
      this.emit("clanInfo", this.clanInfo);
      this.logEvent("clan", "Clan Updated", `Clan ${this.clanInfo.name || ""} state updated.`);
    }

    // 3. Team Changed Broadcast (Player Death / Respawn / Member changes)
    ...
    if (message.broadcast?.entityChanged) {
      ...
      if (this.activeServer) {
        ...
        if (server) {
          ...
          // If Smart Alarm triggered (state === true), send High-Priority Raid Ping & Trigger Compound Lockdown!
          if (isAlarm && state) {
            ...
            if (this.deviceAutomation) {
              this.deviceAutomation.recordAlarmActivation(entityId, entityName);
              this.deviceAutomation.triggerLockdown(entityName, entityId, "alarm").catch(err => {
                console.error("[Lockdown] Error triggering lockdown:", err.message);
              });
            }
            if (this.matrixClient) {
              try {
  ...
  async fetchMapMarkers() {
    ...
    // Forward Cargo Ship status to MonumentTracker
    if (this.monumentTracker) {
      const cargoMarker = formattedMarkers.find(m => m.type === 5);
      this.monumentTracker.handleCargoMarker(cargoMarker || null, Boolean(cargoMarker));
    }
  ...
  async pollTeamInfo() {
    ...
    try {
      ...
      console.warn("[RustPlus] Error polling team info:", e.message);
    }
  }
  ...
  async pollTimeInfo() {
    ...
    try {
      ...
      console.warn("[RustPlus] Error polling time info:", e.message);
    }
  }
  ...
  async pollServerInfo() {
    ...
    try {
      ...
      console.warn("[RustPlus] Error polling server info:", e.message);
    }
  }
  ...
  // Strip emoji/unicode pictographs from strings before sending to in-game chat,
  // because Rust's in-game team chat does not render emojis properly.
  stripEmoji(str) {
    return (str || "")
      .replace(/[\u{1F000}-\u{1FFFF}]/gu, "") // Miscellaneous/Supplemental Symbols and Pictographs
  ...
  async sendTeamChat(message) {
    ...
    const safeMsg = this.stripEmoji(message);
    return new Promise((resolve, reject) => {
      this.client.sendTeamMessage(safeMsg, (res) => {
        if (res?.response?.error) {
  ...
  getStatus() {
    return {
      ...
      clanInfo: this.clanInfo,
      monuments: this.monumentTracker ? this.monumentTracker.getStatus() : null,
      recentPairCodes: this.recentPairCodes || [],
