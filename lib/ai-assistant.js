...
class AIAssistant {
  ...
  buildSystemContext(source = "game") {
    ...
    // 7. Base Team Notes (Door codes, coordinates) - excluded in clan chat for security
    const notes = source === "clan" ? "" : Array.from(this.savedNotes.entries()).map(([k, v]) => `${k}: ${v}`).join("; ");

    // 8. Clan Armory & Boom Reserves
    let armoryInfo = "No storage monitors connected";
    if (this.rustClient.storageTracker) {
      try {
        const arm = this.rustClient.storageTracker.getClanArmoryTotals();
        const exp = arm?.explosives || {};
        const sulfur = (arm?.totalSulfurPower || 0).toLocaleString();
        armoryInfo = `${exp.rockets || 0}x Rockets, ${exp.c4 || 0}x C4, ${exp.satchels || 0}x Satchels, ${exp.exploAmmo || 0}x Explo 5.56 | Total Raid Sulfur Potential: ${sulfur} Sulfur`;
      } catch (e) {
        armoryInfo = "Storage info unavailable";
      }
    }

    // 9. Compound Defense Lockdown Status
    let lockdownStatus = "Normal (Inactive)";
    if (this.rustClient.deviceAutomation) {
      const st = this.rustClient.deviceAutomation.getLockdownStatus();
      if (st.active) lockdownStatus = `🚨 LOCKDOWN ACTIVE (${st.reason})`;
    }

    // 10. Monument CCTV Camera Directory (Official Facepunch monument codes)
    const monumentCameras = `Large Oil Rig: OILRIG1 (Helipad), OILRIG2 (Crane), OILRIG3 (Exhaust), OILRIG4 (Moonpool), OILRIG5 (Lower Deck), OILRIG6 (Top Deck) | Small Oil Rig: OILRIG01 (Helipad), OILRIG02 (Crane), OILRIG03 (Lower Walkway), OILRIG04 (Dock) | Dome: DOME1 (Top Exterior), DOME2 (Walkway Mid), DOME3 (Lower Ground) | Airfield: AIRFIELD1-AIRFIELD4 | Launch Site: LAUNCHSITE1-LAUNCHSITE4 | Water Treatment: WATERPLANT1-WATERPLANT3 | Train Yard: TRAINYARD1-TRAINYARD3 | Military Tunnels: MILITARYTUNNELS1-MILITARYTUNNELS2 | Bandit: BANDIT1-BANDIT2 | Outpost: OUTPOST1-OUTPOST2`;

    // 11. Clan Custom Directives from WebUI Settings
...
- Monument Timers (Oil Rigs & Cargo): ${this.rustClient.monumentTracker ? this.rustClient.monumentTracker.formatChatSummary() : "Unavailable"}
- Clan Armory & Boom: ${armoryInfo}
- Compound Defense & Lockdown: ${lockdownStatus}
...
- Monument CCTV Cameras: ${monumentCameras}
...
- When asked about monument CCTV camera codes (e.g. Oil Rig, Dome, Launch), provide the exact camera identifier(s) from the directory.
- When asked about raid costs or raiding readiness, calculate the exact rockets or C4 needed and check against the Clan Armory.
- Keep answers ultra-compact so they fit in in-game team chat or Matrix.`;
  ...
  async ask(question, source = "game") {
    ...
    try {
      const systemPrompt = this.buildSystemContext(source);
