...
class StorageTracker {
  constructor(rustClient, matrixClient) {
    ...
    if (this.rustClient) this.initListeners();
  }
  ...
  initListeners() {
    ...
    this.rustClient.on("entityChanged", ({ entityId, payload }) => {
      ...
      const isAlarmOrSwitch = server && (
        (server.alarms || []).some(a => Number(a.id) === eId && a.type !== "storage") ||
        (server.switches || []).some(s => Number(s.id) === eId && s.type !== "storage")
      );
      if (isAlarmOrSwitch) return;

      const isKnownStorage = server && (server.storageMonitors || []).some(s => Number(s.id) === eId);
      if (payload && (isKnownStorage || (payload.capacity && payload.capacity > 0))) {
  ...
  getTcLabel(name) {
    ...
    if (upper === "TC") return "TC (Main Base)";
    if (upper === "TCFARM" || upper === "TC_FARM" || upper === "TC-FARM") return "TCFarm (Farm Base)";
    const extMatch = upper.match(/^TC_?EXT_?(\d+)$/i);
    if (extMatch) return `TCExt${extMatch[1]} (External ${extMatch[1]})`;
    return n;
  }

  getPairedStorageMonitors() {
    if (!this.rustClient.activeServer) return [];
    // Check server storageMonitors and any alarms marked as storage/tc
    const server = this.rustClient.activeServer;
  ...
  updateContainerData(entityId, payload) {
    ...
    // Check decay - only if it is actually a Tool Cupboard!
    const isTC = payload.capacity === 24 || 
                 (payload.protectionExpiry && payload.protectionExpiry > 0) ||
                 pairedName.toLowerCase().includes("tc") || 
                 pairedName.toLowerCase().includes("cupboard");

    if (isTC && payload.hasProtection === false && !this.decayAlertSent.has(eId)) {
  ...
  getUpkeep(nameOrId) {
    ...
    const list = tcs.map(tc => {
      const label = this.getTcLabel(tc.name);
      if (tc.protectionExpiry) {
        ...
        return `"${label}": ${wood}w, ${stone}s, ${metal}m, ${hqm}hqm`;
      }
    }).join(" | ");
  ...
  getState() {
    const containerList = Array.from(this.containers.values()).map(c => {
      ...
      const tcLabel = isTc ? this.getTcLabel(c.name) : null;
      if (c.protectionExpiry) {
      ...
      const resources = {
        wood: (c.items || []).find(i => i.shortname === "wood")?.quantity || 0,
        stone: (c.items || []).find(i => i.shortname === "stones")?.quantity || 0,
        metal: (c.items || []).find(i => i.shortname === "metal.fragments")?.quantity || 0,
        hqm: (c.items || []).find(i => i.shortname === "metal.refined")?.quantity || 0
      };
      return {
        ...c,
        isTc,
    ...
    const tcs = containerList.filter(c => c.isTc);
