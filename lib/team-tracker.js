...
class TeamTracker {
  ...
  getAlertConfig() {
    ...
    if (this.rustClient?.configManager && typeof this.rustClient.configManager.getConfig === "function") {
      const cfg = this.rustClient.configManager.getConfig();
      return { ...defaultCfg, ...(cfg.teamAlerts || {}) };
    }
    return defaultCfg;
  }

  setAlertConfig(updates) {
    if (this.rustClient?.configManager && typeof this.rustClient.configManager.saveConfig === "function") {
      const cfg = this.rustClient.configManager.getConfig() || {};
      cfg.teamAlerts = { ...this.getAlertConfig(), ...updates };
      this.rustClient.configManager.saveConfig(cfg);
      return cfg.teamAlerts;
    }
    return this.getAlertConfig();
  }

  formatOfflineHours(hours) {
  ...
  async triggerReconnectAlert(name, steamId, offlineHours, lastSeenTime) {
    const cfg = this.getAlertConfig();
    const hoursStr = this.formatOfflineHours(offlineHours);
    const intHours = Math.floor(offlineHours);
    const mins = Math.floor((offlineHours - intHours) * 60);
    const detailedDuration = mins > 0 ? `${hoursStr} (${intHours}h ${mins}m)` : hoursStr;

    const message = `Team member ${name} is now online after being offline for ${hoursStr}.`;
    const inGameMessage = `🟢 Team member ${name} is now online after being offline for ${hoursStr}.`;
    const title = `🟢 Teammate Online: ${name}`;

    console.log(`[TeamTracker] 🟢 Reconnect Alert: "${message}" (SteamID: ${steamId}, Offline: ${detailedDuration})`);

    // 1. WebUI Event Log & WebSocket broadcast
    if (this.rustClient && typeof this.rustClient.logEvent === "function") {
      this.rustClient.logEvent("team", title, inGameMessage, {
        steamId,
        name,
        offlineHours: Math.round(offlineHours * 10) / 10,
        offlineDuration: detailedDuration,
        lastSeen: this.formatTimeAgo(lastSeenTime)
      });
    }

    // 2. In-game Team Chat
    if (cfg.inGameTeamChat && this.rustClient?.client?.isConnected()) {
      try {
        await this.rustClient.sendTeamChat(inGameMessage);
      } catch (err) {
        console.warn("[TeamTracker] In-game team chat failed:", err.message);
      }
    }

    // 3. Facepunch Clan Chat (for large 65-80 player clans)
    if (cfg.clanChat && this.rustClient?.clanInfo?.clanId && this.rustClient?.client?.isConnected()) {
      try {
        await this.rustClient.sendClanMessage(inGameMessage);
      } catch (err) {
        console.warn("[TeamTracker] In-game clan chat failed:", err.message);
      }
    }

    // 4. Matrix E2EE Team Chat & Alerts Room
    if (cfg.matrixAlerts && this.rustClient?.matrixClient) {
      try {
        if (typeof this.rustClient.matrixClient.sendTeamChat === "function") {
          await this.rustClient.matrixClient.sendTeamChat("Rust+ Sentinel", inGameMessage, "#10b981");
        }
        if (typeof this.rustClient.matrixClient.sendAlert === "function") {
          await this.rustClient.matrixClient.sendAlert(title, message, {
            "Player": name,
            "SteamID": steamId,
            "Offline Duration": detailedDuration,
            "Last Seen": this.formatTimeAgo(lastSeenTime),
            "Server": this.rustClient.activeServer?.name || "Active Server"
          });
        }
      } catch (err) {
        console.warn("[TeamTracker] Matrix alert dispatch failed:", err.message);
      }
    }
  ...
  processTeamInfo(teamInfo) {
    ...
    for (const m of teamInfo.members) {
      ...
      if (!prev) {
        const wentOfflineAt = m.isOnline ? null : (this.wipeStats[steamId]?.lastSeen || now);
        const lastSeenOnline = m.isOnline ? now : (this.wipeStats[steamId]?.lastSeen || null);

        // If initial sync has already completed and a player appears as online for the first time
        if (this.isInitialSyncDone && m.isOnline && this.wipeStats[steamId]?.lastSeen) {
          const wentOfflineTime = this.wipeStats[steamId].lastSeen;
          if (now > wentOfflineTime) {
            const offlineHours = (now - wentOfflineTime) / 3600000;
            const cfg = this.getAlertConfig();
            if (cfg.enabled && offlineHours >= (cfg.minOfflineHours !== undefined ? cfg.minOfflineHours : 1.0)) {
              this.triggerReconnectAlert(name, steamId, offlineHours, wentOfflineTime).catch((err) => {
                console.error("[TeamTracker] Error triggering reconnect alert:", err.message);
              });
            }
          }
        }
      ...
      if (!prev.isOnline && m.isOnline) {
        ...
        const wentOfflineTime = prev.wentOfflineAt || prev.lastSeenOnline || this.wipeStats[steamId]?.lastSeen;
        let offlineHours = 0;
        if (wentOfflineTime && now > wentOfflineTime) {
          offlineHours = (now - wentOfflineTime) / 3600000;
        }
